...
def build_firstPart_model(input_var,keep_prob_=0.5):
    #
    filters = 128
    networkadd = tf.layers.conv1d(inputs=input_var, filters=filters, kernel_size=50, strides=6,
                             padding='same', activation=tf.nn.relu)
#    print(networkadd)
    network = tf.nn.dropout(networkadd, keep_prob_)
    #
...
def main(args=None):
    ...
    tf.app.flags.DEFINE_string('output_dir', 'outputs_2013/outputs_eeg_fpz_cz(conv)',
    ...
    tf.app.flags.DEFINE_string('checkpoint_dir', 'checkpoints-seq2seq-sleep-EDF(conv)', """Directory to save checkpoints""")
