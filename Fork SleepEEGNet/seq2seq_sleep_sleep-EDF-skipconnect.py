...
#        tf.nn.atrous_conv2d(input_var,filters=64,rate=2,padding='same'）
        network1 = tf.layers.max_pooling1d(inputs=network, pool_size=8, strides=8, padding='same')

        # Dropout
        network = tf.nn.dropout(network1, keep_prob_)
...
#                                 padding='same', activation=tf.nn.relu)
        ...
        network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=8, strides=1,
                                 padding='same', activation=tf.nn.relu)
        network = tf.add(network1, network)
        ...
        network2 = tf.layers.max_pooling1d(inputs=network, pool_size=4, strides=4, padding='same')

        # Dropout
        network = tf.nn.dropout(network2, keep_prob_)
...
#                                   padding='same', activation=tf.nn.relu)
        ...
        network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=6, strides=1,
                                   padding='same', activation=tf.nn.relu)
        network = tf.add(network2, network)
        # Max pooling
...
def main(args=None):
    ...
    tf.app.flags.DEFINE_string('output_dir', 'outputs_2013/outputs_eeg_fpz_cz(skipconnect)',
    ...
    tf.app.flags.DEFINE_string('checkpoint_dir', 'checkpoints-seq2seq-sleep-EDF(skipconnect)', """Directory to save checkpoints""")
