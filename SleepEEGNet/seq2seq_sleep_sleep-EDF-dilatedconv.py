...
def build_firstPart_model(input_var,keep_prob_=0.5):
    #
    filters = 5
    dilated_conv = tf.layers.Conv1D(filters=filters,kernel_size=25,padding='same',dilation_rate=2)
    networkadd = dilated_conv.apply(inputs=input_var)
    print(networkadd)
    networkadd = tf.nn.dropout(networkadd, keep_prob_)
    #
    network = tf.layers.conv1d(inputs=networkadd, filters=filters, kernel_size=8, strides=1,
                             padding='same', activation=tf.nn.relu)
    network = tf.layers.conv1d(inputs=network, filters=filters, kernel_size=8, strides=1,
                             padding='same', activation=tf.nn.relu)
    network = tf.layers.conv1d(inputs=network, filters=filters, kernel_size=8, strides=1,
                             padding='same', activation=tf.nn.relu)
    #
    network = tf.add(networkadd, network)
...
def main(args=None):
    ...
    tf.app.flags.DEFINE_string('output_dir', 'outputs_2013/outputs_eeg_fpz_cz(dilatedconv)',
    ...
    tf.app.flags.DEFINE_string('checkpoint_dir', 'checkpoints-seq2seq-sleep-EDF(dilatedconv)', """Directory to save checkpoints""")
